...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const ProductArrivalDates = {
        ...
        if (data.status && typeof data.status === 'string' && data.status.toLowerCase() === 'success') {
            ...
            if (data.data && data.data.arrivalDates && Array.isArray(data.data.arrivalDates)) {
               ...
               if (validDates.length > 0) {
                    this.state.arrivalDates = validDates.sort((a, b) => {
                    ...
                    console.log(`PA Dates: Processed ${this.state.arrivalDates.length} valid arrival date(s).`);
    ...
    renderStandardLayout: function(datesToRender) {
        ...
        let html = `<div class="arrival-dates-header"><h3 class="arrival-dates-title">${this.sanitizeText(title)}</h3></div><ul class="arrival-dates-list">`;
        datesToRender.forEach(item => {
            html += `<li class="arrival-date-item">
                        <div class="arrival-date">${this.formatDate(item.eta)}</div>
                        <div class="arrival-qty">${this.sanitizeText(item.qty)} units</div>
                        <div class="arrival-status">${this.getEtaTypeLabel(item.etaType)}</div>
                     </li>`;
        });
        html += '</ul>';
        const note = this.config.noteStandard || '<strong>Note:</strong> Dates subject to change.';
        if (note) {
            html += `<div class="arrival-dates-note"><p>${note}</p></div>`;
        }
    ...
    renderCompactLayout: function(datesToRender) {
        ...
        const maxVisible = this.config.maxVisibleCompact || 1;
        const totalDates = datesToRender.length;

        let html = `<div class="arrival-dates-subtle__title">${this.sanitizeText(title)}</div><ul class="arrival-dates-subtle__list">`;
        datesToRender.forEach((item, index) => {
            const isHidden = index >= maxVisible ? 'style="display: none;"' : '';
            html += `<li class="arrival-dates-subtle__item" ${isHidden}>
                        <div class="arrival-dates-subtle__date">${this.formatDate(item.eta)}</div>
                        <div class="arrival-dates-subtle__qty">(${this.sanitizeText(item.qty)})</div>
                        <div class="arrival-dates-subtle__status">${this.getEtaTypeLabel(item.etaType)}</div>
                     </li>`;
        });
        html += '</ul>';

        if (totalDates > maxVisible) {
            const remaining = totalDates - maxVisible;
            const showMoreText = (this.config.labelShowMore || 'Show {count} more date{s}')
                .replace('{count}', remaining)
                .replace('{s}', remaining > 1 ? 's' : '');
            html += `<button type="button" class="arrival-dates-subtle__toggle" data-action="show-more">${this.sanitizeText(showMoreText)}</button>`;
        }

        if (note) {
            html += `<div class="arrival-dates-subtle__note">${this.sanitizeText(note)}</div>`;
        }
        this.elements.content.innerHTML = html;
    ...
    addToggleListener: function() {
        const toggleButton = this.elements.content.querySelector('.arrival-dates-subtle__toggle');
        if (toggleButton) {
            toggleButton.addEventListener('click', (e) => {
                e.preventDefault();
                const action = e.target.dataset.action;
                const listItems = this.elements.content.querySelectorAll('.arrival-dates-subtle__item');
                const maxVisible = this.config.maxVisibleCompact || 1;

                if (action === 'show-more') {
                    listItems.forEach(item => item.style.display = 'flex');
                    const showLessText = this.config.labelShowLess || 'Hide extra dates';
                    e.target.textContent = this.sanitizeText(showLessText);
                    e.target.dataset.action = 'show-less';
                } else {
                    ...
                    const remaining = listItems.length - maxVisible;
                    const showMoreText = (this.config.labelShowMore || 'Show {count} more date{s}')
                        .replace('{count}', remaining)
                        .replace('{s}', remaining > 1 ? 's' : '');
                    e.target.textContent = this.sanitizeText(showMoreText);
                    e.target.dataset.action = 'show-more';
                }
            });
        }
    },
